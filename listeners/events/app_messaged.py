...
def app_messaged_callback(client: WebClient, event: dict, logger: Logger, say: Say):
    ...
    logger.info(f"[app_messaged] Received message from user {user_id} in channel {channel_id}")
    ...
    try:
        if event.get("channel_type") == "im":
            ...
            if thread_ts:  # Retrieves context to continue the conversation in a thread.
                ...
                logger.info(f"[app_messaged] Parsed {len(conversation_context)} messages from thread")

            logger.info(f"[app_messaged] Sending waiting message...")
            waiting_message = say(text=DEFAULT_LOADING_TEXT, thread_ts=thread_ts)
            logger.info(f"[app_messaged] Waiting message sent with ts: {waiting_message.get('ts')}")
            ...
            logger.info(f"[app_messaged] Received response from provider (length: {len(response)})")
        ...
        logger.error(f"[app_messaged] ERROR: {type(e).__name__}: {str(e)}", exc_info=True)
        if waiting_message:
            try:
                client.chat_update(
                    channel=channel_id,
                ...
                logger.error(f"[app_messaged] Failed to update error message: {update_error}", exc_info=True)
